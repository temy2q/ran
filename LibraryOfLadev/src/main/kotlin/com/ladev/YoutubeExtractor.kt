 ...
 *
 * This follows the same pattern as re-3rabi's Yextractor:
 * 1. NewPipeExtractor fetches the YouTube page and extracts all stream URLs
 *    (it handles signature decryption, poToken, etc. automatically)
...
open class YoutubeExtractor : ExtractorApi() {
    ...
    override suspend fun getUrl(
        ...
        val videoId = try {
            cleanedUrl.substringAfter("v=").substringBefore("&")
        } catch (e: Exception) {
            cleanedUrl
        }
            ...
            try {
                ...
                for (video in videoOnlyList) {
                    if (audiosByLanguage.isNotEmpty()) {
                        ...
                        // No audio tracks — emit video-only with DASH
                        val dashXml = buildDashManifestXml(video, emptyList(), durationSeconds)
                        val localLink = registerManifestAndGetUrl(dashXml)
    ...
    private fun buildVideoLabel(vs: org.schabi.newpipe.extractor.stream.VideoStream): String {
